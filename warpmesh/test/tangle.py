...
def check_dataset_tangle(dataset, model,
                         n_elem_x, n_elem_y,):
    """
...
def plot_prediction(data_set, model, prediction_dir,
                    mode, n_elem_x, n_elem_y, loss_fn,
                    savefig=True
                    ):
    num_data = len(data_set)
    for idx in range(num_data):
        val_item = data_set[idx]
        plot_sample(model, val_item, prediction_dir,
                    loss_fn, n_elem_x, n_elem_y, idx, mode, savefig)


def plot_sample(model, val_item, prediction_dir,
                loss_fn, n_elem_x, n_elem_y, idx, mode, savefig=True):
