...
for i in {$n_grid_start..$n_grid_end}; do
  if ((i % $stride == 0)); then
    echo "n_grid = $i"
    python ./script/build_helmholtz_square.py --n_grid=$i  --rand_seed=$rand_seed --n_samples=$n_samples --field_type="aniso" --boundary_scheme="full"
    python ./script/build_helmholtz_square.py --n_grid=$i  --rand_seed=$rand_seed --n_samples=$n_samples --field_type="iso" --boundary_scheme="pad"
    # python ./script/build_helmholtz_square.py --n_grid=$i  --rand_seed=$rand_seed --n_samples=$n_samples --field_type="iso" --boundary_scheme="full"
    # python ./script/build_helmholtz_square.py --n_grid $i  --rand_seed $rand_seed --n_samples $n_samples --field_type "aniso" --boundary_scheme "pad"
  fi
done