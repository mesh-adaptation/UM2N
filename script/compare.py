...
error_reduction_ma = np.mean(
    (np.array(error_og) - np.array(error_ma)) / np.array(error_og))  # noqa
error_reduction_model = np.mean(
    (np.array(error_og) - np.array(error_model)) / np.array(error_og)) # noqa
...
mrn_loss_data_filtered = mrn_loss_data[(mrn_loss_data['Train Loss'] <= 50) & (mrn_loss_data['Test Loss'] <= 50)]  # noqa
m2n_loss_data_filtered = m2n_loss_data[(m2n_loss_data['Train Loss'] <= 50) & (m2n_loss_data['Test Loss'] <= 50)]  # noqa
...
ax1.plot(m2n_loss_data_filtered['Epoch'], m2n_loss_data_filtered['Train Loss'], label='M2N Train Loss', color='cyan', lw=2, alpha=0.65)  # noqa
ax1.plot(m2n_loss_data_filtered['Epoch'], m2n_loss_data_filtered['Test Loss'], label='M2N Test Loss', color='blue', lw=2, linestyle='--', alpha=0.65)  # noqa
ax1.plot(mrn_loss_data_filtered['Epoch'], mrn_loss_data_filtered['Train Loss'], label='MRN Train Loss', color='red', lw=2, alpha=0.65)  # noqa
ax1.plot(mrn_loss_data_filtered['Epoch'], mrn_loss_data_filtered['Test Loss'], label='MRN Test Loss', color='orange', lw=2, linestyle='--', alpha=0.65)  # noqa
...
ax1.annotate(f'M2N: {final_test_loss_m2n:.2f}', (final_epoch_m2n, final_test_loss_m2n - 3), textcoords="offset points", xytext=(0,0), ha='center', fontsize=20, color='blue', weight='bold')  # noqa
ax1.annotate(f'MRN: {final_test_loss_mrn:.2f}', (final_epoch_mrn, final_test_loss_mrn + 3), textcoords="offset points", xytext=(0,0), ha='center', fontsize=20, color='orange', weight='bold')  # noqa
...
ax2.plot(tangle_m2n_data_filtered['Epoch'], tangle_m2n_data_filtered['Test Tangle'], label='M2N Test Tangle', color='blue', lw=2)  # noqa
ax2.plot(tangle_mrn_data_filtered['Epoch'], tangle_mrn_data_filtered['Test Tangle'], label='MRN Test Tangle', color='orange', lw=2)  # noqa
...
ax2.annotate(f'M2N: {final_test_tangle_m2n:.4f}', (final_epoch_m2n_tangle, final_test_tangle_m2n + 0.07), textcoords="offset points", xytext=(10,0), ha='center', fontsize=20, color='blue', weight='bold')  # noqa
ax2.annotate(f'MRN: {final_test_tangle_mrn:.4f}', (final_epoch_mrn_tangle, final_test_tangle_mrn + 0.02), textcoords="offset points", xytext=(10,0), ha='center', fontsize=20, color='orange', weight='bold')  # noqa
ax2.set_ylim(0, 1)
...
def summrise_info(res):
    error_reduction_ma = np.mean(
        (res["error_og"] - res["error_ma"]) / res["error_og"])
    error_reduction_model = np.mean(
...
dataset_list = [
    f'/Users/cw1722/Documents/irp/irp-cw1722/data/dataset/helmholtz/z=<0,1>_ndist=None_max_dist=6_<{n_node}x{n_node}>_n=40_cmplx/train' for n_node in [18, 19, 20, 21, 22]  # noqa
]
