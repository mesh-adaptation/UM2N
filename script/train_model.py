...
# write to csv
df.to_csv(
    os.path.join(output_dir, 'info.csv'),
    index=False)
#  ==============================================================
...
if __name__ == "__main__":
    ...
    with Live(progress, refresh_per_second=5):  # update 10 times a second
        for epoch in range(num_epochs):
            ...
            train_loss = wm.train(train_loader, model, optimizer, device,
                                  loss_func=loss_func, use_jacob=use_jacob)
            test_loss = wm.evaluate(test_loader, model, device,
                                    loss_func=loss_func, use_jacob=use_jacob)
    ...
    df_train.to_csv(
        os.path.join(trainlog_dir, 'loss.csv'),
        index=False)
    ...
    df_tangle.to_csv(
        os.path.join(trainlog_dir, 'tangle.csv'),
        index=False)
