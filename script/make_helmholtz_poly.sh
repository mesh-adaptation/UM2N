...
for i in {$n_grid_start..$n_grid_end}; do
    echo "lc = $i"
    python ./script/build_helmholtz_poly.py --lc=$i  --rand_seed=$rand_seed --n_samples=$n_samples_test --field_type="aniso" --boundary_scheme="full"
    python ./script/build_helmholtz_poly.py --lc=$i  --rand_seed=$rand_seed --n_samples=$n_samples_test --field_type="iso" --boundary_scheme="pad"
    python ./script/build_helmholtz_poly.py --lc=$i  --rand_seed=$rand_seed --n_samples=$n_samples_test --field_type="iso" --boundary_scheme="full"
    python ./script/build_helmholtz_poly.py --lc $i  --rand_seed $rand_seed --n_samples $n_samples_test --field_type "aniso" --boundary_scheme "pad"
  fi
done